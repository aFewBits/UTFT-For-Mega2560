...
*/
#define ILI9327_8BIT		// this forces ILI9327 8 bit mode 
/*
--------------------------------------------------------------------
 So long as ILI9327_8BIT is defined, UTFT is forced into the correct 
 configuration for the MCUFRIEND 2560 LCD Shield.
 
 The base ILI9327 driver is used with several modifications to operate
 properly as an 8 bit driver. UTFT does not work with this driver if 
 you simply  set the display mode to 8 bit mode as the "Writ_bus" routines
 will incorrectly write two bytes to the controller.
 
 Using the #ifdef constructs very clearly delineate where modifications
 have been made to UTFT, which are minimal. The following files have been
 modified: 

	utft.cpp
	utft.h
	\hardware\avr\hw_atmega1280.h
	\tft_drivers\ili9327\initlcd.h
	\tft_drivers\ili9327\setxy.h

The examples directorys have been trimmed to remove the drivers that
are not of interest with the mcufriend shield as well as the screen
resolutions that are not supported by the mcufriend shield.
The base UTFT code remains unchanged so techincally the examples would 
work if you had the corresponding hardware but they are extra code "noise"
that is excess baggage for our purposes.
---------------------------------------------------------------------
*/
...
// Include hardware-specific functions for the correct MCU
	...
	#include "hardware/avr/HW_AVR.h"
		...
		#pragma message ("Compiling for ATmega1289/2560")
...
#if defined ILI9327_8BIT
		cbi(P_RS, B_RS);
		LCD_Writ_Bus(VL,display_transfer_mode);
#else
	if (display_transfer_mode!=1)
	{
		cbi(P_RS, B_RS);
